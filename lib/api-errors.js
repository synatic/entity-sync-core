...
export function formatApiError(response, action, context = {}) {
  ...
  // Always include the raw body so generic messages like "Internal server
  // error" still expose stack traces / nested fields from the API.
  if (response.body !== undefined && response.body !== null && response.body !== "") {
    const raw =
      typeof response.body === "string" ? response.body : stringifyDetail(response.body);
    if (raw) {
